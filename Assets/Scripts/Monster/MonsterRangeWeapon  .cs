...
public class MonsterRangeWeapon : MonsterWeaponHandler
{
    ...
    public override void Attack()
    {
        if( ProjectileManager == null)
        {
            ...
            if (ProjectileManager == null)
            {
                Debug.LogError("[MonsterRangeWeapon] ProjectileManager가 NULL. 씬에 MonsterProjectileManager가 있는지 확인하세요.", this);
                return;
            }
    ...
    public void CreateProjectile(Vector2 _lookDirection, float angle)
    {
        ...
        {
            Debug.LogError("[MonsterRangeWeapon] projectileSpawnPosition이 NULL. 인스펙터에서 설정하세요.", this);
            return;
        }

        if (_base == null)
        {
            Debug.LogError("[MonsterRangeWeapon] _base가 NULL. Init 호출 여부 확인하세요.", this);
            return;
        }
