...
public class Monster_Ranged_Controller : MonoBehaviour
{
...
#region 무기 (weaponhandler)
    ...
    private IEnumerator RandomAttackCoroutine()
    {
        while (true)
        {
            ...
            if (weaponHandler != null && GameObject.FindWithTag("Player") is GameObject player)
            {
                lookDirection = (player.transform.position - transform.position).normalized;

                isAttacking = true;
                yield return new WaitForSeconds(0.1f); // 잠시 true 유지
                isAttacking = false;
            }
        }
    }
